// =============================================================================
// U10 — Economy & Rewards Module (ES5 JavaScript)
// =============================================================================
...
function readPlayerInventory(nk, userId) {
  ...
  if (result && result.length > 0) {
    var loaded = result[0].value;
    for (var k in loaded) { defaults[k] = loaded[k]; }
    // Ensure nested shields defaults exist
    if (loaded.shields) {
...
function claimDailyLoginRpc(ctx, logger, nk, _payload) {
  ...
  if (stats.last_login_claim > 0 && (now - stats.last_login_claim) >= TWO_DAYS_MS) {
    if (stats.active_streak_shields > 0) {
      stats.active_streak_shields = 0; // Consume the shield
...
function spinWheelRpc(ctx, logger, nk, payload) {
  ...
  // Probability distribution (25 segments)
  ...
  if (roll < 45) {
    rolledRarity = "Common";
    var subRoll = Math.floor(Math.random() * 4);
    if (subRoll === 0) { rewardCoins = 30; displayMessage = "30 Coins"; }
    else if (subRoll === 1) { rewardCoins = 50; displayMessage = "50 Coins"; }
    else if (subRoll === 2) { rewardCoins = 80; displayMessage = "80 Coins"; }
    else { rewardXP = 15; displayMessage = "+15 XP Boost"; }
  } else if (roll < 73) {
    rolledRarity = "Uncommon";
    var subRoll = Math.floor(Math.random() * 4);
    if (subRoll === 0) { rewardCoins = 100; displayMessage = "100 Coins"; }
    else if (subRoll === 1) { rewardCoins = 150; displayMessage = "150 Coins"; }
    else if (subRoll === 2) { rewardCoins = 200; displayMessage = "200 Coins"; }
    else { rewardItemType = "token"; rewardItemKey = "spin"; displayMessage = "1x Extra Spin Token"; }
  } else if (roll < 88) {
    rolledRarity = "Rare";
    var subRoll = Math.floor(Math.random() * 4);
    if (subRoll === 0) { rewardCoins = 250; displayMessage = "250 Coins"; }
    else if (subRoll === 1) { rewardCoins = 350; displayMessage = "350 Coins"; }
    else if (subRoll === 2) { rewardItemType = "shield"; rewardItemKey = "starter"; displayMessage = "1x Starter Arena Shield"; }
    else { rewardItemType = "shield"; rewardItemKey = "bronze"; displayMessage = "1x Bronze Arena Shield"; }
  } else if (roll < 96) {
    rolledRarity = "Epic";
    var subRoll = Math.floor(Math.random() * 4);
    if (subRoll === 0) { rewardCoins = 500; displayMessage = "500 Coins"; }
    else if (subRoll === 1) { rewardCoins = 700; displayMessage = "700 Coins"; }
    else if (subRoll === 2) { rewardItemType = "shield"; rewardItemKey = "silver"; displayMessage = "1x Silver Arena Shield"; }
    else { rewardItemType = "shield"; rewardItemKey = "gold"; displayMessage = "1x Gold Arena Shield"; }
  } else if (roll < 99) {
    rolledRarity = "Legendary";
    var subRoll = Math.floor(Math.random() * 3);
    if (subRoll === 0) { rewardCoins = 1000; displayMessage = "1000 Coins Jackpot!"; }
    else if (subRoll === 1) { rewardItemType = "shield"; rewardItemKey = "platinum"; displayMessage = "1x Platinum Arena Shield"; }
    else { rewardItemType = "cosmetic"; rewardItemKey = "card_back_neon"; displayMessage = "Neon Card Back Skin"; }
  } else {
    rolledRarity = "Ultra Rare";
    var subRoll = Math.floor(Math.random() * 2);
    if (subRoll === 0) { rewardCoins = 2000; displayMessage = "2000 Coins Jackpot!"; }
    else { rewardItemType = "cosmetic"; rewardItemKey = "cosmetic_crown"; displayMessage = "Golden Crown Accessory"; }
  }

  // Enforce 800c Daily Soft Cap
  ...
  // Enforce Weekly Jackpot Cooldown
  if (rewardCoins >= 1000) {
    var oneWeekMs = 7 * 24 * 60 * 60 * 1000;
    if (limits.last_jackpot_timestamp && (now - limits.last_jackpot_timestamp) < oneWeekMs) {
      logger.info("[Economy] Player " + userId + " hit jackpot cooldown. Downgrading jackpot to Rare reward.");
      rewardCoins = 250;
      displayMessage = "250 Coins (Jackpot Cooldown applied)";
    } else {
  ...
  logger.info("[Economy] Player " + userId + " spun wheel: Rarity=" + rolledRarity + ", Result=" + displayMessage);
  return JSON.stringify({
    success: true,
    rarity: rolledRarity,
...
// ---------------------------------------------------------------------------
// Module entry point — registers economy RPCs
// ---------------------------------------------------------------------------

function initEconomyModule(ctx, logger, nk, initializer) {
